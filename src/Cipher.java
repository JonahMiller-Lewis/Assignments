...
public class Cipher {
	public static void main(String[] args) {
		...
		//Store the total sum of the inputs as an int variable. Outputs the value of the total
		int total = firstNumber + secondNumber + thirdNumber + fourthNumber + fifthNumber;
		System.out.println("Total = " + total);
		
		//Creates a random generator that is used to assign a value to an int variable which represents the key.
		//Outputs the value of that key
		Random keyGen = new Random();
		int key = keyGen.nextInt(9);
		System.out.println("Your Random key is " + key);
		
		//Assigns to new int variables which are the separates tens and the ones place of the sum
		int tensPlace = total/10;
		int onesPlace = total%10;
		
		//Encodes both the tens place and the ones place, then assigns a new int variable as the combination of the two
		...
		//The encoded number is then output
		if (encodedNumber < 10) {
